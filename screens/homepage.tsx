...
export default function HomePageAudio(props: RouteStackParamList<'HomePage'>) {
      ...
      axios(config)
        .then((json: any) => { 
          ...
          AsyncStorage.getItem('hashId').then((hashId)=>{
            setPosts([...getPosts(postsInside, hashId)])
          })
        })
    })
  ...
  function getPosts (postsInsideFunc: any, loginUserId: any){
    let arr: any = [];
    ...
    Posts.map((e, index) => {
      ...
      arrayOfComponents.push(
        <VideoPost
          ...
          follower={e.follower}
          navigation={props.navigation}></VideoPost>
      );
    });
  }
  return (
    <View style={styles.background}>
      <HomeTitle {...(props as unknown as RouteStackParamList<'HomePage'>)} title="Following" />
